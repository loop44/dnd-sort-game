...
const Game = () => (
  <GamePageWrapper>
    <Image src={backgrounds[randomTheme]} alt="" priority fill placeholder="blur" />
    <GamePageInner>
      <Playground>
        {items.map((item) => {
          const randomItemBg = randomItemsTheme[Math.round(0 - 0.5 + Math.random() * (3 - 0 + 1))];
          return <Image src={randomItemBg} alt="" key={item} />;
        })}
      </Playground>
      <Dock>
